...
const PyTorchDeployManual: React.FC<Props> = ({ service }) => {
    ...
    <div className='rounded shadow px-10 py-10 mt-5 bg-white'>
      <div className='flex gap-8'>
          ...
          <div className='opacity-20'>
            ...
            <div style={{ fontFamily: 'Inconsolata' }}  className='bg-black text-white rounded py-3 px-4'>
             <span className='text-pink-600' style={{ fontFamily: 'Inconsolata' }}>import</span> inferrd<br/>
              ...
              inferrd.deploy_torch(model, <span className='text-green-600' style={{ fontFamily: 'Inconsolata' }}>'{service.name}'</span>)
            </div>
          </div>
        </div>
        ...
        <div className='flex-1'>
          ...
          <div className='text-gray-600 my-3'>Save your PyTorch model using the <a className='text-blue-600' target='_blank' href='https://docs.inferrd.com/environments/using-spacy'><InlineCode>.save</InlineCode> method</a> format. Exporting a model will create a file, upload that file to deploy the model.</div>

          <div className='text-gray-600 my-3'>Run this cell in your notebook to export your model to a folder. Replace <InlineCode>model</InlineCode> with the variable containing the PyTorch model.</div>

          <div style={{ fontFamily: 'Inconsolata' }} className='bg-black text-white rounded py-3 px-4'>
            torch.save(model, <span className='text-green-600' style={{ fontFamily: 'Inconsolata' }}>'model.pt'</span>)
          </div>

          <DeployZone service={service} title='Drag and drop the model.pt here'/>
        </div>
      </div>
    </div>
  )
...
export default PyTorchDeployManual